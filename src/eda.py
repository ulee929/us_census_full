...
class Analysis():
    ...
    def pop_columns(self):
        '''Pops continuous variables to separate dataframe'''
        df_cont = pd.DataFrame([self.data.pop(x) for x in self.columns]).T
        return df_cont

    def replace_outliers_with_means(self):
        '''Replace outlier values of continuous variables with their means'''
